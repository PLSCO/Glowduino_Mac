...
int uText::setFont(prog_uchar font[]) {
    int p1 = pgm_read_byte_near_macro(font + 0);
    int p2 = pgm_read_byte_near_macro(font + 1);
    if ( p1 != 'Z' || p2 != 'F' ) {
...
//			Serial.println( p2 );
    ...
    int fontType = pgm_read_byte_near_macro(font + 2);
...
void uText::printString(int16_t xx, int16_t yy, String text, int clean, int8_t kerning[]) {
    ...
    int fontType = pgm_read_byte_near_macro(currentFont + 2);
    if ( fontType != ANTIALIASED_FONT && fontType != BITMASK_FONT ) {
        return;
    }
    ...
    int glyphHeight = pgm_read_byte_near_macro(currentFont + 3);
    ...
    for (int t = 0; t < text.length(); t++) {
        ...
        while ( 1 ) {
            char cx = (char)(((int)pgm_read_byte_near_macro(currentFont + ptr + 0) << 8) + pgm_read_byte_near_macro(currentFont + ptr + 1));
            if ( cx == 0 ) {
              break;
            }
            int length = (((int)(pgm_read_byte_near_macro(currentFont + ptr + 2) & 0xff) << 8) + (int)(pgm_read_byte_near_macro(currentFont + ptr + 3) & 0xff));
...
//						Serial.println( " glyph definition. Font corrupted?" );
                ...
                width = 0xff & pgm_read_byte_near_macro(currentFont + ptr + 4);

                int marginLeft = 0x7f & pgm_read_byte_near_macro(currentFont + ptr + 5);
                int marginTop = 0xff & pgm_read_byte_near_macro(currentFont + ptr + 6);
                int marginRight = 0x7f & pgm_read_byte_near_macro(currentFont + ptr + 7);
                ...
                if ( fontType == ANTIALIASED_FONT ) {
                    ...
                    boolean vraster = (0x80 & pgm_read_byte_near_macro(currentFont + ptr + 5)) > 0;
                    ...
                    if ( vraster ) {
                        ...
                        for ( int i = 0; i < length - 8; i++ ) {
                            int b = 0xff & pgm_read_byte_near_macro(currentFont + ptr + 8 + i);
                            int x = ctr / effHeight;
                            int y = ctr % effHeight;
                    ...
                    boolean compressed = (pgm_read_byte_near_macro(currentFont + ptr + 7) & 0x80) > 0;
                    if ( compressed ) {
                        boolean vraster = (pgm_read_byte_near_macro(currentFont + ptr + 5) & 0x80) > 0;
                        if ( vraster ) {
                            int marginBottom = marginRight;
                            int effHeight = glyphHeight - marginTop - marginBottom;

                            for ( int i = 0; i < length - 8; i++ ) {
                                int len = 0x7f & pgm_read_byte_near_macro(currentFont + ptr + 8 + i);
                                boolean color = (0x80 & pgm_read_byte_near_macro(currentFont + ptr + 8 + i)) > 0;
                                if ( color ) {
                                    int x = ctr / effHeight;
                                    int y = ctr % effHeight;
                                    while ( y + len > effHeight ) {
...
int16_t uText::getLineHeight() {
    ...
    return pgm_read_byte_near_macro(currentFont + 3);
}
...
int16_t uText::getBaseline() {
    ...
    return pgm_read_byte_near_macro(currentFont + 4);
}
